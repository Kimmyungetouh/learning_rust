use std::io;
fn fahrenheit_to_celsius(temperature:u16) -> f64{
    let result:f64 = ((temperature - 32) * (5 / 9)) as f64;
    return result;
}

fn celsius_to_fahrenheit(temperature:u16) -> f64{
    let result:f64 = (((temperature) * 9 / 5) + 32) as f64;
    return result;
}
...
fn main() {
    ...
    let temperature:u16 = match temperature.trim().parse::<u16>() {
        Ok(number) => number,
